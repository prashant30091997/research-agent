"""PubMed Search + lightweight Europe PMC metadata enrichment.
Step 1: Search PubMed for papers (titles, authors, abstracts)
Step 2: For each paper, get PMCID + fill missing authors/abstracts from Europe PMC
PDF URL finding happens later in the downloader, not here.
"""
import httpx
import asyncio
...
async def _enrich_with_europmc(client, papers: list):
    """Lightweight enrichment: only grab PMCID and fill missing abstracts/authors.
    NO PDF URL lookup here — that happens in the downloader."""
    for paper in papers:
        doi = paper.get("doi", "")
        pmid = paper.get("pmid", "")
